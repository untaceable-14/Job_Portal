...
if(isset($_POST['submit'])){
  ...
  $select_admin = $conn->prepare("SELECT * FROM `companies` WHERE adminname = ?");
  ...
  if($select_admin->rowCount() > 0){  
    ...
    if($pass != $cpass){
      ...
      $insert_admin = $conn->prepare("INSERT INTO companies(company_name, adminname, adminpassword) VALUES(?,?,?)");
      $insert_admin->execute([$com_name, $name, $cpass]);
...
<section class="form-container">
  ...
  <form action="" method="post">
   <img src="../images/ecommerce logo.png" width="250px" height="130px" >
      ...
      <input style="border: .2rem solid #444444;" type="text" id="admin_inp" name="com_name" required placeholder="enter company name" maxlength="20"  class="box" oninput="this.value = this.value.replace(/\s/g, '')">
      <input style="border: .2rem solid #444444;" type="text" id="admin_inp" name="name" required placeholder="enter admin name" maxlength="20"  class="box" oninput="this.value = this.value.replace(/\s/g, '')">
